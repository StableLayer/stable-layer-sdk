...
function e2eTestnetMintBurnEnabled(): boolean {
  return (
    ...
    Boolean(
      process.env.E2E_TESTNET_PRIVATE_KEY?.trim() || process.env.SUI_PRIVATE_KEY?.trim(),
    )
  );
}
...
function unwrapTxResult(result: unknown): { digest?: string; success?: boolean } {
  const r =
    (result as { Transaction?: { digest?: string; status?: { success?: boolean } } })
      .Transaction ?? result;
...
describe.skipIf(!e2eTestnetMintBurnEnabled())("testnet mint → burn (live chain)", () => {
  it(
    "mints then burns the same USDC raw amount",
    { timeout: 180_000 },
    async () => {
